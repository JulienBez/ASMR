...
def sortByDistances(similarity_threshold=0.1,LANG=parameters.NAMEPATH):
    """for each seed, find the closest sentences to this seed (a.k.a remove the furthest ones)"""

    ids = []
    sentences = []

    for path in glob.glob(f"data/{LANG}/{parameters.TEST_DATA}.json"):
        data = openJson(path)
    ...
    sequences = list(openJson(f"data/{LANG}.json").keys())
...
def sortByCommons(common_elements=3,LANG=parameters.NAMEPATH):
    ...
    for path in glob.glob(f"data/{LANG}/{parameters.TEST_DATA}.json"):
        data = openJson(path)#[:100]#LIMIT FOR TEST

        for i in data:
            sentences.append(i["parsing"]["lemma"])
            ids.append(i["metadata"]["id"])

    sequences = list(openJson(f"data/{LANG}.json").keys())#[:10] #LIMIT FOR TEST
    dict_seq = {}
...
def sortBySeeds(LANG=parameters.NAMEPATH):
    """for each sentence, sort it with each seed that it is relativly close to"""
    
    if parameters.PROCESS_VERSION == "sortByDistances":
        dict_seq = sortByDistances(LANG=LANG)

    elif parameters.PROCESS_VERSION == "sortByCommons":
        dict_seq = sortByCommons(LANG=LANG)
        ...
        for path in glob.glob(f"data/{LANG}/{parameters.TEST_DATA}.json"):
            ...
            for i in range(len(data)):
                ...
                for key,values in data[i]["parsing"].items():
                    data[i]["parsing"][key] = [v if v != "-" else "--" for v in values] #MIGHT CAUSE ISSUES

        if len(new_data) > 0:
            writeJson(f"output/{LANG}/sorted/{''.join(x for x in seq.replace(' ','_').replace('¤','#') if x.isalnum() or x == '_')}.json",new_data)


def process(LANG=parameters.NAMEPATH):
    """process process on every file in 'data/' folder"""
    deleteFolderContent(f"output/{LANG}/sorted")
    createFolders(f"output/{LANG}/sorted")
    sortBySeeds(LANG=LANG)
    print("")
